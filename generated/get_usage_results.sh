...
for d in  generated_test_blur_3x3_sliding_window_kerne*; do
	echo "Dir: $d"
	rpt=$d/_x/reports/link/imp/impl_1_full_util_placed.rpt
	axibits=$(echo $d | awk -F "v0_" '{print $2}' | awk -F "b_" '{print $1}')
	vect=$(echo $d | awk -F "v0_" '{print $2}' | awk -F "vectorize_" '{print $2}')
	if [ -z $vect ]; then
		vect="no"
	fi
